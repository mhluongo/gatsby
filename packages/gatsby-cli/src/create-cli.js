...
function buildLocalCommands(cli, isLocalSite) {
  ...
  function showVersionHeader() {
    ...
    const sharpVersion = (isGatsbySite && getLocalSharpVersion()) || `none`
    report.info(
      `Using: gatsby-cli@${cliVersion}, gatsby@${gatsbyVersion}, sharp@${sharpVersion}, nodejs@${getCurrentNodeVersion()}`
    )
  }
...
function getVersionInfo(cliVersion) {
  ...
  if (isGatsbySite) {
    ...
    const sharpVersion = getLocalSharpVersion() ?? `not installed`
